...
async function handleFloorDrawClick(messageId) {
    ...
    if (resolvedMessageId !== messageId) {
        console.warn('[ComfyDraw] 楼层面板 messageId 与 DOM 不一致，已按 DOM 楼层生成:', { state: messageId, dom: resolvedMessageId });
...
export function ensureComfyDrawPanel(messageEl, messageId, options = {}) {
    const settings = getSettings();
...
export function setStateForMessage(messageId, state, data = {}) {
    ...
    if (!panelData?.root?.isConnected) {
        ...
        if (messageEl) {
            panelData = ensureComfyDrawPanel(messageEl, messageId, { force: true });
        }
    }
    if (panelData) {
...
function createFloatingButton() {
    ...
    floatingEl.id = 'nd-comfy-floating-global';
