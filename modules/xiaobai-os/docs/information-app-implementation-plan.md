...
- 主聊天手动改删不反写 APP、不自动复活楼层；APP 当前通讯支持统一删除与最新一轮重新回复，历史不可改，详见[修改与恢复设计](./information-app-modifications.md)。不提供消息编辑、版本切换或整段回退。
...
domains/messages/
  ...
  commands.ts              联系人、追加消息、回复提交
  mutation.ts              删除、整组替换、摘要失效
  migrations/              正式线 v1 冻结格式及入口迁移
...
采用当前 `MessagesDomainV2` 规范快照，不复制 Game/Tasks 的事件链。包含联系人、完整消息、连续通讯片段和单份待确认修改；摘要属于联系人。正式线 v1 只在解析入口转换，运行时不携带旧语义。

- 联系人只有稳定身份、姓名、必要识别说明/备注；不保存人设全文和世界书 UID。候选名单从当前只读人物资料得到，不另存候选缓存。
- 消息具有稳定 ID、联系人归属、全分区单调序号、发送方、发生时间和类型化内容。投影按片段 `messageIds` 展示；重 roll 的新序号不改变原轮次位置。`seq` 只负责分配及确认高水位，不重编号或复用。
...
分区字段为 `version/nextSeq/contacts/messages/segments/pendingMutation`。`replyTo` 表达触发关系；片段记录展示顺序、封存、补录与回执。回执 `throughSeq/digest` 确认成员高水位对应的投影，不承诺原生楼层仍存在。解析校验真实成员和正文摘要，拒绝损坏数据；重 roll 后按 `messageIds` 顺序重算摘要、高水位取最大序号。
...
统一删除先验证完整可写楼层，经跨存档确认后移除消息和空片段、更新投影回执；只删一部分时保持当前片段可续写。删除联系人必须全部记录均满足权限，其他联系人不受影响。

图片不再有专用删除接口。任意消息删除都只移除所选记录，保留后续回复并清除指向它的引用；覆盖该消息的摘要清空，其他联系人摘要保留。图库文件不删除。序号不复用，不新增墓碑或回滚快照。
...
回执仍在片段中。唯一新增持久态是 Messages 所有的 `pendingMutation`，用于跨重启确认一次跨文件修改；它只含目标、原楼校验凭据、删除 ID、新回复及候选摘要。成功或安全撤销后立即删除，清分区时一并删除。`createdHere/observedClosed`、运行代次、UI 权限和草稿仍是临时态；不设全局锁、任务表、完整副本或版本历史。
...
人物事实投影保留已有关系状态 `trend`，以及 `since` 对应的事实更新楼层（展示为从 1 开始的楼号）。没有更新标记时不补造，不把首次出现的 `_addedAt` 当作更新时间。仍使用原有 8000 字符预算，不新增存储或向量召回。
...
请求按“较早通讯 → 剧情间隔 → 唯一一份当前剧情记忆 → 本次通讯”装配。`application/communication-chronology.ts` 用片段成员和主聊天投影标记临时还原通讯位置；两段之间有普通主剧情才标明剧情间隔，其他联系人的私人楼层、系统消息或仅因总结封存而另起片段不构成剧情推进。楼层只表达先后，不换算故事内时长；原楼删除、重复标记、缺少投影或补录无法证明原时点时明确标为未知。当前输入尚未投影也按本次捕获的剧情位置装配。

补录片段只承载此前未确认同步的旧消息，不接受后续新短信；新短信另起普通通讯片段，按实际主剧情位置定位。另起片段本身不构成剧情间隔，连续交流在重启后仍保持同一通讯时点。

`prompt/communication-history.ts` 由回复与压缩共用。断点留在重新联系的第一条短信之前，后续连续短信归入同一段；剧情事件、人物弧光和关系事实从原来的历史前置位置移到本次通讯前，只注入一份。旧通讯摘要仍是短信替代材料，不是另一份剧情总结。所有定位仅存在于请求内，不增字段、不改短信或原生楼层正文；预览、计量、发送与重生成采用同一投影。

当前记忆采用按序的资料消息而非 `system` 消息，避免 Anthropic/Google/Responses 适配器将其提取到开头。人物身份和资料优先级仍在系统提示词内；用量统计复用同一背景构造，不按消息位置或角色猜测背景归属。

### 6.4 长线程

采用“较早通讯摘要＋近期原文”。摘要只归 Messages 线程，不写进世界书或 Story Summary，不作为新消息投影到主聊天。

压缩输入保留通讯分段与剧情位置，合并摘要时保持各阶段的态度、约定和先后关系。注入摘要时根据仍保留的原文重新附上其覆盖范围及进入末段前的断点，即使断点两侧短信已被压缩，也不把整个摘要当成当前连续对话；未知位置不补造。摘要的定位标注保持有界，不重新枚举全部已压缩阶段而抵消压缩效果。
...
上下文采用与四次元壁一致的 token 口径：整包输入达到 128k 时，在回复前压缩较早通讯；158k 为整包输入上限。计量包含提示词、角色/世界书、剧情/人物资料、旧摘要、未压缩通讯和当前输入，复用通用 tokenizer，不可用时估算。每张实际附图另预留 6k（不是精确视觉 token）。保留最近至少五个已回答轮次及十条原文，不切开保留轮次；当前输入不摘要。每批摘要请求也计量并限制在 128k 内，旧摘要仍至多 6000 字符。仅完整且确实缩减上下文的摘要才确认并推进覆盖序号；失败不推进该批范围、不删原文。压缩后仍超过 158k 则明确停止回复，不静默裁掉近期记录。128k 是触发线，不是硬上限；暂无较早可压缩材料且不超过 158k 时可正常回复。

宿主背景仍有独立范围（近期四条、每条 4000 字符、剧情事件至多 20000 字符等）：Messages 每次捕获读取总结当前过滤配置，用其同一清洗函数先清洗主剧情原文，再截取；世界书扫描也使用清洗后的剧情。既保留默认规则，也即时采用用户自定义/移除规则，不改 Map/Tasks 默认行为。阈值不等于模型实际支持窗口，仍需选择支持足够上下文的模型。

会话顶部提供同款用量圆环，点开显示整包估算分项及 128k 自动总结规则，草稿本地追加计量。`messages/context` 仅做只读预览，不下载附图、不调用回复模型、不写分区；切联系人、退出或版本变化后丢弃迟到结果。统计不依赖 UI 分页，不新增持久化计数器或摘要格式。
...
- 回复的语气、措辞、情感表达与内容共同承接人物设定和记忆；剧情已确立的事实与关系发展优先于初始设定中的旧状态。旧短信与通讯摘要保留当时言行，其后的剧情发展同样影响当前通讯，旧问题不自动成为本轮话题。亲历或已获知的总结内容属于人物记忆，其他人的秘密不因此变成已知；缺少总结仍可依据现有设定与对话交流。
...
上述是普通发送的追加投影路径。删除/重 roll 另先保存单份修改候选，并读取已保存聊天核对原楼/目标结果，确认后才应用 APP 差量；删除使用原生尾楼删除生命周期，不重建被删除楼层。
...
专属 renderer 只处理有合法 APP 标记及正文结构的楼层；显示为原生 `details/summary` 通讯记录。一律默认折叠，标题显示联系人与真实消息数，折叠预览取末条消息的前 96 码点，超长加省略号。折叠时不挂载消息气泡；每次展开显示最新 40 个记录节点并置底。同一 DOM 容器、同一片段更新时保留展开选择和阅读位置。

气泡严格保持原始先后，只按连续联系人分组；单联系人标题不重复，多联系人 A→B→A 仍为三段。方向由左右布局区分，无障碍标签保留通讯双方。正文区高度上限为 600px / 65% 动态视口的较小值，短记录不撑高；内部独立滚动，阻止滚动穿透。向前/后每次加载 40 个记录节点，同时最多挂载 80 个，保留重叠窗口和可见记录偏移；新消息只在原本位于最新底部时跟随，翻历史不抢滚动，提供“回到最新”。图片加载后按同一规则恢复位置。外观继承宿主字体与颜色，采用细分隔线、低对比气泡与淡绿发出底色。

文本按不可信输入处理，不把模型 HTML 插入 DOM；本地附件继续校验路径，图片描述/语音原文仍可读。编辑中的原生楼层不替换，无变化的 DOM 不重建。展开选择仅存于弱引用 DOM 状态，不新增持久字段；卸载/渲染失败仍显示可读原文。

展示层分为宿主楼层适配（`message-renderer.ts`）、窗口与阅读位置（`message-record-view.ts`）、安全气泡渲染（`message-record-content.ts`）。尺寸观察仅在展开时存活，折叠或 DOM 移除后断开；不增加跨 APP 状态或注册入口。2026-09-13 已用一万条隔离记录验证限量渲染、内部置底、双向翻页、新消息/宿主重绘保位、图片延迟加载及移动端深浅主题；不改变通讯原文和 RP/总结输入。
...
只保留仍有消息的联系人，清空备注和摘要：这两者只有父线最新快照，无法证明其历史版本；之后按保留原文正常生成摘要。没有通讯证据的通讯录条目不带入。继承片段全部封存，回执按保留的已确认成员重算；保留 `nextSeq` 高水位，避免序号复用。完整复制不裁剪历史，但和分支一样先按子聊天的真实投影解析待确认修改，再清除父线操作。父线及其他 APP 不变，不新增分支快照库。
...
- 普通发送重试遇到已有确认回复：只返回既有回复/补同步，不再请求模型；明确的「重新回复」另走权限与版本校验，替换最新整组，不逐气泡重抽。
...
图片、文字、语音统一经 `messages/message/delete` 请求删除；旧图片专用接口删除。可写权限由 Host 下发且执行前重验。确认保存后权威刷新当前加载范围，不跳首屏、不复活旧分页记录；生成/待确认时禁删，检查保存不发模型请求。媒体错误不再建议删除不可修改的历史图片；「重试」仍仅处理最后一条待回复输入。

共享图片缓存使用 `os-messages` namespace，只有本地七天有效期；过期可按描述再生成，不能承诺永久/跨设备保持同一张图。沿用酒馆的返回契约：共享 facade 返回裸 PNG Base64 时补 `data:image/png;base64,`，已有 PNG/JPEG/WebP/GIF data URL 保持原格式；缓存命中使用同一转换，不加载模型任意远程地址。删除联系人/分支/APP 仅清其分区，不连带删除共享缓存。播放句柄、AbortController、进度和缓存状态不进分区；关闭页面取消媒体，但不取消通讯回复。

图片进入通讯滚动区的视野后自动读取缓存，未命中才生成；屏幕外的旧图不提前发起生成。`ui/MessageImage.vue` 独立管理显示、放大、排队/生成进度和失败重试，重试不重新请求人物回复。图片请求用临时请求 ID 关联进度与取消，切联系人/卸载气泡只取消自己的请求；旧请求不能取消或覆盖重进后的新请求。具体绘图错误原样展示；桥接等待超过三分钟时明确报超时并取消对应任务，避免界面已失败而后台继续占用队列。图片仅显示失败时重新加载已有图片，不重抽图。
