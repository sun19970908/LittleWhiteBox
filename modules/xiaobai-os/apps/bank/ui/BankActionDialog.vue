...
<template>
    <AppDialog class="bank-dialog" :aria-label="title" :busy="busy" @close="emit('cancel')">
        <form @submit.prevent="submit">
            ...
            <template v-if="fundProduct"><dl class="bank-dialog-summary"><div><dt>整期收益区间</dt><dd>{{ fundProduct.returnLabel }}</dd></div><div><dt>风险等级</dt><dd>{{ fundProduct.riskLabel }}</dd></div></dl><p class="bank-dialog-warning">可能损失本金。申购后不能提前退出，实际收益到期后揭晓。</p></template>
