...
export function buildWorldDataMessage(world: WorldContent): string {
    ...
    // With the domain field limits, all IDs, titles and the overview fit
    // even at maximum JSON escaping. Only this outgoing copy loses article bodies.
    data.news = data.news.map(item => ({ ...item, body: '' }));
    return render('Current world publication as reference data. Article bodies are omitted to fit the context budget; empty body fields here do not describe the saved articles. Overview, IDs and titles are complete.');
}
