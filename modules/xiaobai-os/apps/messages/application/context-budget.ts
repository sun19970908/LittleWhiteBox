...
import { threadLine, type buildReplyPrompt } from '../prompt/reply-prompt.js';
...
export function estimateContext(prompt: Prompt, contact: MessageContact, history: PrivateMessage[]): MessagesContextStats {
    const usedTokens = estimateConversationTokens({ messages: conversation(prompt) }) + imageReserve(prompt);
    const backgroundTokens = estimateConversationTokens({ messages: prompt.messages.slice(0, 2) });
