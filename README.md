...
16. 直证组内不再渲染L0行（L0与所属事件摘要语义重复，同源派生必然重叠），开关hideL0UnderEvents默认隐藏，需要循环任务（已附在循环任务目录）
17. 直接/远期证据池硬上限从5000放开到50000（面板向量配置 summarizedEvidenceBudget 可调），解决 direct evidence 大量被预算跳过的问题
