...
## 兼容tt虚拟化：
- 阻断了participant 注册实现了tauritavern的虚拟化时酒馆助手和小白x的部分兼容，需要二改的tauritavern和循环任务开关，并增加一个剧情总结的QR按钮
- 解锁了虚拟化时小白x的按钮，实测draw没问题，别的不清楚
...
- 可选屏蔽l3（定了的事）里的核心事实相关人物（state约束），防止长聊天l3太多，需要循环任务
- L3 人物屏蔽功能（可以按主体屏蔽 + 完全屏蔽 world + 完全屏蔽 people），需要循环任务
- 剧情总结成功后出弹窗（防止llm有幻觉便于人工检查）

## 向量召回优化：
- 修复向量召回词法零命中的问题，lexical加了个截断防止lexical太多
